...
class Dashboard extends Component {
	...
	editKeyChain = (websiteName, websiteUrl, email, password, id) => {
		axios
			.put(`/api/cred/:${id}`, { websiteName, websiteUrl, email, password, id })
			.then(() => {
				...
				<div key={e.cred_id}>
					<div className='dashboarditemmain'>
						<div className='dashboarditem'>
							<p className='keyChainitem'>{e.website_name}</p>
							<p className='keyChainitem'>{e.website_url}</p>
							<p className='keyChainitem'>{e.username}</p>
							<p className='keyChainitem' onClick={this.state.toggleShow}>
								{e.password}
							</p>
							<p className='keyChaindate'>{Date(e.update_time)}</p>
						</div>
						<div className='edit__dropdown'>
							<button onClick={() => this.editKeyChain(e.cred_id)}>Edit</button>
							<button onClick={() => this.deleteKeyChain(e.cred_id)}>
								delete
							</button>
						</div>
					</div>
